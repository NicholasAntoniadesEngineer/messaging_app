// Auth page bootstrap (sign in / sign up / password reset / recovery / device pairing).
// H-5: externalized from auth.html inline <script> so script-src can drop
// 'unsafe-inline'. Loaded with `defer`; the top-level wiring below runs after the
// document is parsed (all referenced elements already exist, identical to the
// original parse-time-at-end-of-body execution). The former trailing
// DOMContentLoaded listener is converted to a readyState-guarded call so it still
// runs once even if DOMContentLoaded has already fired by the time this executes.
...
function showRecoveryKeyModal(recoveryKey, onConfirm) {
    return new Promise((resolve) => {
        ...
        downloadBtn.onclick = () => {
            const blob = new Blob([`Secure Messenger Recovery Key\n\nSave this recovery code in a safe place:\n\n${currentRecoveryKey}\n\nDate: ${new Date().toISOString()}\n\nIMPORTANT: This is the ONLY way to recover your encrypted messages if you forget your password.`], { type: 'text/plain' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `secure-messenger-recovery-key-${Date.now()}.txt`;
            a.click();
            URL.revokeObjectURL(url);
        };

        printBtn.onclick = () => {
            // FE-03: HTML-escape the recovery key before it is written into
            // the print document. Self-XSS only (the user prints their own
            // key), but cheap to close.
            ...
            printWindow.document.write(`<html><head><title>Secure Messenger Recovery Key</title><style>body{font-family:Arial,sans-serif;padding:40px;}h1{color:#333;}.recovery-key{font-family:'Courier New',monospace;font-size:16px;line-height:2;background:#f5f5f5;padding:20px;border:2px solid #333;margin:20px 0;}.warning{color:#d32f2f;font-weight:bold;}</style></head><body><h1>Secure Messenger Recovery Key</h1><p class="warning">KEEP THIS SAFE AND SECURE</p><p>If you forget your password, this recovery code is the ONLY way to recover your encrypted messages.</p><div class="recovery-key">${escapeHtml(currentRecoveryKey)}</div><p>Date: ${new Date().toISOString()}</p></body></html>`);
...
function showEncryptionSuccess() {
    ...
    setTimeout(() => {
        window.location.href = '../../messaging/views/messenger.html';
    }, 1000);
}
...
async function handlePostSignIn() {
    ...
    try {
        ...
        try {
            ...
            // This is NOT a fresh device - do not sign the user out and do not
            // jump straight to the recovery-key screen. Fall through to the
            // password-backup restore path, which re-establishes a usable
...
document.getElementById('pair-device-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (result && result.success) {
            okEl.textContent = 'Paired! Loading your messages…';
            okEl.classList.remove('hidden');
            setTimeout(() => { window.location.href = '../../messaging/views/messenger.html'; }, 800);
        } else {
...
// --- Reset Encryption (lost recovery key) ---
// Highest-integrity, NO-backdoor escape hatch. There is no admin recovery (that would
// defeat E2E). When a user has lost their recovery key after a password reset, this
// re-keys with a fresh identity: the account is kept, server identity + backup rows are
// overwritten (upsert), and old messages encrypted to the previous identity become
// permanently unreadable. The user is shown a NEW recovery key to save.
document.getElementById('reset-encryption-btn').addEventListener('click', async () => {
    clearErrors();
    const confirmed = window.confirm(
        'Reset encryption?\n\n' +
        'This creates brand-new encryption keys for your account. You KEEP your account, but every ' +
        'existing encrypted message becomes permanently unreadable — no one, including an administrator, ' +
        'can recover them without your recovery key.\n\n' +
        'You will be shown a NEW recovery key to save. Continue?'
    );
    if (!confirmed) return;

    const userId = document.getElementById('recovery-key-restore-section').dataset.userId;
    const btn = document.getElementById('reset-encryption-btn');
    btn.disabled = true;

    try {
        const password = window.PasswordManager?.retrieve();
        if (!password) {
            showError('recovery-restore-error', 'Session expired — please sign in again, then reset.');
            await window.AuthService.signOut();
            return;
        }
        ...
        // Wipe local key state so regeneration is clean (preserves the at-rest wrap key).
        if (window.KeyStorageService?.clearAll) {
            await window.KeyStorageService.clearAll();
        }

        // Fresh identity — upserts overwrite the old (now-undecryptable) server identity.
        const keyResult = await window.KeyManagementService.generateAndStoreIdentityKeys(userId);
        if (!keyResult || !keyResult.success) {
            throw new Error('Failed to generate new encryption keys');
        }
        ...
        await showRecoveryKeyModal(recoveryKeyFormatted, async (finalRecoveryKey) => {
            ...
            // Overwrites the stale backup with one keyed to the NEW identity.
            await window.KeyManagementService.createDualBackup(password, recoveryKeyToUse);
            window.PasswordManager?.clear();
            document.getElementById('recovery-key-modal').style.display = 'none';
            showEncryptionSuccess();
        ...
        console.error('[Auth] Reset encryption failed:', error);
        showError('recovery-restore-error', 'Reset failed. Please try again.');
        btn.disabled = false;
    }
});
